...
### 新增 (Added) - 后置质检 skill 与结构化 qc_report

- **新增 `quality-check` 技能**：补充独立的后置质检 skill，用于读取正式 `index / decision / evidence / record` 结果包并输出结构化 `qc_report`，不再依赖自然语言摘要做人工判断。
- **新增质检报告写入脚本**：新增 `quality-check/scripts/write_qc_report.py`，统一执行结果包结构检查、跨文件一致性检查、证据支撑检查和修正闭环检查，并将结果稳定落盘到 `output/qc_results/{task_id}/qc_report_<timestamp>.json`。
- **新增质检报告校验器与 schema**：补充 `quality-check/scripts/validate_qc_report.py` 与 `quality-check/schema/qc_report.schema.json`，确保质检报告的 `overall / checks / metrics / source_bundle` 结构稳定可复用。
- **技能文档同步补齐**：新增 `quality-check/SKILL.md`，明确输入是正式 `index_*.json`，唯一正式输出是 `qc_report_path`。
- **README 同步更新**：仓库说明现已补充 `quality-check` 的定位、输出目录与推荐执行顺序。

### 影响范围

- `quality-check/SKILL.md`
- `quality-check/schema/qc_report.schema.json`
- `quality-check/scripts/qc_common.py`
