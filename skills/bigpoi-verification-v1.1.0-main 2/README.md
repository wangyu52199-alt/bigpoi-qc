...
本仓库用于维护大 POI 核实与结果质检技能及其脚本，覆盖证据收集、核实决策、结果成包、最终规格校验与后置质检。

官方 Claude Code Skills 文档：<https://code.claude.com/docs/zh-CN/skills>

## 仓库内容

当前仓库包含四个技能，其中前三个组成核实主链路，第四个用于后置质检：
...
- `quality-check/`：质检技能，负责读取正式结果包并输出结构化 `qc_report`
...
- `timestamp` 格式固定为 `yyyyMMddTHHmmssZ`
- `quality-check` 只读取正式 `index / decision / evidence / record`，不以 `output/runs/{run_id}` 下的过程文件作为正式判断依据
- `qc_report` 需要包含 `overall`、`checks`、`metrics` 与 `source_bundle`
- 正式报告必须通过 `quality-check/scripts/validate_qc_report.py` 校验
...
```bash
python quality-check/scripts/write_qc_report.py -IndexPath <output/results/{task_id}/index_<timestamp>.json> -WorkspaceRoot <repo-root> [-PoiPath <input.json>]
python quality-check/scripts/validate_qc_report.py -ReportPath <output/qc_results/{task_id}/qc_report_<timestamp>.json>
```
...
1. 将四个技能目录统一整理到项目级 `.claude/skills/` 或用户级 `~/.claude/skills/` 下
2. 保持跨技能脚本引用仍基于同一仓库根目录，避免拆分部署后相对路径失效
