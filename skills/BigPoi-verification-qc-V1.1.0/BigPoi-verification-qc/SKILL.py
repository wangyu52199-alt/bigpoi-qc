...
# -*- coding: utf-8 -*-
"""
BigPOI QC Skill v2.4.3 - 单入口执行器

固定流程：
1. 接收模型维度草稿（仅 task_id + dimension_results）
...
def _build_qc_draft(payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    dimension_results = _extract_dimension_results(payload)
    if dimension_results is None:
        raise ValueError('缺少 dimension_results：模型只能输出维度级草稿结果。')
