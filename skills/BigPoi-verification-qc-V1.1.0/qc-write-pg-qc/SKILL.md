---
name: qc-write-pg-qc
description: 从本地化JSON文件读取大POI质检结果，回写到PostgreSQL指定表（v1.2.3 新增回库前结果校验；继续保留受约束递归恢复并对齐 downgrade_consistency）
metadata:
  version: "1.2.3"
...
6. 如果找到多个合法候选，不自动猜测，直接返回歧义错误
7. 对加载出的 `qc_result` 执行结果校验，确保结构、评分和统计标记自洽
8. 写入到指定的表（默认 poi_qc_zk，或自定义表）
