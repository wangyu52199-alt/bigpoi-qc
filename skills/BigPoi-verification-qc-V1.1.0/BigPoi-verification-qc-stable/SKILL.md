---
name: bigpoi-verification-qc-stable
version: 2.4.10-stable
description:
  对上游大POI核实结果进行确定性质量检验，官方输入固定为上游平铺结构。重点检查名称、坐标、地址、行政区划、类型、存在性、证据充分性，以及人工核实降级是否一致。
  输出结构化、可审计、可复算的质检结果。
metadata:
  entrypoint_path: ./SKILL.py
...
# QC Skill · Big POI Verification v2.4.10-stable
...
10. 必须调用 `./SKILL.py` 作为唯一执行入口；`SKILL.py` 内部固定执行：`finalize_qc_result.py -> result_validator.py -> result_persister.py`
11. 若 `SKILL.py` 返回校验失败或持久化失败，视为本次质检失败，不得继续回库
...
- 手工编写顶层 `explanation`
- 让模型直接拼装最终 `qc_result`
- 跳过 `./SKILL.py`，或绕过 `./SKILL.py` 直接手工拼接最终结果文件

### 5.1 单入口执行（强制）

必须通过 `./SKILL.py` 执行，不得临时创建 `run_qc.py`、`temp_*.py` 等脚本。

命令行示例：

```bash
python SKILL.py <payload.json> [output_dir]
```

`payload.json` 约定：

- 必须包含：`task_id`（可放在平铺输入或顶层）与 `dimension_results`
- 推荐包含：上游平铺输入字段（`name/address/x_coord/y_coord/poi_type/city/verify_result/evidence_record`）
- 可选包含：`model_judgement`（hybrid 覆盖建议）

`SKILL.py` 只采纳维度草稿字段，并忽略模型手工填写的 `qc_status/qc_score/risk_dims/triggered_rules/statistics_flags/explanation` 等最终聚合字段。
...
在声明“质检结果已保存”之前，必须已经成功调用 `./SKILL.py` 且返回 `success=true`，并且返回路径必须来自执行器真实输出，不得自行拼接。
`./SKILL.py` 在真正写入任何文件前，必须先执行 `finalize_qc_result.py` 和 `result_validator.py`；校验不通过时禁止落盘。

### 9.1 本地持久化要求

如果需要将质检结果落盘，必须使用 `./SKILL.py`（由其内部调用 `result_persister.py`），不得自行约定目录和文件名。
