---
name: qc-write-pg-qc
description: 从本地化JSON文件读取大POI质检结果，回写到PostgreSQL指定表（v1.3.0 引入统一运行时配置文件 qc_runtime.json，支持固定结果路径）
metadata:
  version: "1.3.1"
...
| 版本 | 日期 | 说明 |
|------|------|------|
| 1.3.1 | 2026-03-31 | 文档版本同步：`SKILL_FULL.md` 与紧凑版 `SKILL.md` 版本号对齐；能力无行为变更 |
...
8. 只有当多个最新候选在时间戳和修改时间上都无法区分先后时，才返回歧义错误
9. 对加载出的 `qc_result` 先执行派生字段收敛，再做结果校验，确保结构、评分和统计标记自洽
10. 写入到指定的表（默认 poi_qc_zk，或自定义表）
