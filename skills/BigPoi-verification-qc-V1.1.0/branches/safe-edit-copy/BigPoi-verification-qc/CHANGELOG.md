...
### 文档
- `SKILL.md` 压缩为紧凑版，删除重复规则解释和长示例，保留执行闭环与硬约束
- 原完整版说明保留为 `SKILL_FULL.md`，便于追溯

## [2.4.3] - 2026-03-31

### 新增
- 新增 `scripts/evidence_preprocessor.py`，将“无效证据过滤 + source_type 规范化 + 轻量字段补齐”抽为独立可复用模块
- 新增 `scripts/dsl_executor.py`，支持对平铺输入按 `rules/decision_tables.json` 直接执行 6 个核心维度（R1-R6）判定

### 调整
- `scripts/result_contract.py` 的 `finalize_qc_result()` 新增核心维度重算能力：
- 新增参数 `raw_input`、`recompute_core`、`dsl_path`、`preprocess_evidence`
- 默认 `recompute_core=true`，当可提取到平铺输入时，先调用 `dsl_executor` 强制重算核心维度，再执行后续 finalize 收敛
- `scripts/finalize_qc_result.py` 新增 CLI 参数：
- `--raw-input`、`--no-recompute-core`、`--dsl`、`--no-preprocess-evidence`

### 文档
- `SKILL.md` 升级到 `v2.4.3`，执行流程明确为“DSL 先判核心维度，模型仅处理争议维度”
