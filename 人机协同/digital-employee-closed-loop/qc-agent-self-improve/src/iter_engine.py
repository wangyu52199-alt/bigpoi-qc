...
from shared.schemas import IterationExecutionResult, IterationOperation, QCAgentImprovementRecord, SecondRoutingResult
...
def run_qc_auto_iteration(
    ...
    improvement = QCAgentImprovementRecord.model_validate(improvement_payload)
    second = SecondRoutingResult.model_validate(second_routing_payload)
    cfg = _load_config(config_path)

    target_root = Path(target_skill_path).resolve()
    paths = cfg.get("paths", {})
    bucket_key = str(cfg.get("rules_bucket_key", "auto_qc_rules"))

    rule_file = target_root / str(paths.get("rule_file", "config/qc_rules.yaml"))
    ...
    if second.primary_module in QC_MODULES:
    ...
    auto_rule = {
        "id": f"qc-auto-{improvement.sample_id}",
        "module": primary_module,
        "intercept_issue_type": improvement.intercept_issue_type,
        "issue_summary": improvement.issue_summary,
        "evidence_risk": improvement.evidence_risk,
    ...
    log_entry = (
        ...
        f"- issue_type: {improvement.intercept_issue_type}\\n"
