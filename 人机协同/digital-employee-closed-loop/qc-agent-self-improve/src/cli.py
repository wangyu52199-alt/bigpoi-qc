...
from improver import build_qc_improvement_record


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build qc agent improvement record")
...
def main() -> int:
    ...
    result = build_qc_improvement_record(manual_payload, first_payload, second_payload)

    output_path = Path(args.output)
    output_path.parent.mkdir(parents=True, exist_ok=True)
    output_path.write_text(result.model_dump_json(indent=2), encoding="utf-8")
    print(f"[OK] qc improvement record saved: {output_path}")
    return 0
